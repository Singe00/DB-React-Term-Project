...
export default function Edit() {
	...
	const [Selected, setSelected] = useState(state[0].reviewGameKey);
	const [Selected2, setSelected2] = useState(state[0].reviewScore);
	const [textValue, setTextValue] = useState(state[0].reviewDetail);
	const [titleV, settitleV] = useState(state[0].reviewTitle);
	...
	const onhandlePost = async (data) => {
		const { rn, star, title, reviewtext, userID } = data;
		const postData = { rn, star, title, reviewtext, userID };

		// post
		await axios
			.post('/api/edit', postData)
	...
	const handleSubmit = (event) => {
		...
		if (joinData.title !== '' && textValue !== '') {
			const { rn, star, title, reviewtext, userID } = joinData;
	...
	return (
		<div>
			<form onSubmit={handleSubmit}>
				<label>게임 선택</label>
				<select className="optreview" value={Selected} id="opt" name="opt" disabled>
				...
				<input
					...
					maxLength="50"
					value={titleV}
					onChange={handleSetValue2}
				/>
				<label>별점</label>
				<select
					onChange={handleSelect2}
					value={Selected2}
					className="optreview2"
					id="star"
					name="star"
				>
